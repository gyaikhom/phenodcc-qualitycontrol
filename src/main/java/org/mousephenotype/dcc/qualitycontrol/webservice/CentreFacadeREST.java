...
import org.mousephenotype.dcc.qualitycontrol.webservice.pack.CentrePack;
...
public class CentreFacadeREST extends AbstractFacade<ACentre> {
    ...
    public CentrePack extjsFind(
        ...
        if (isValidSession(sessionId, userId)) {
            ArrayList<ACentre> t = new ArrayList<ACentre>();
            t.add(super.find(id));
            p.setDataSet(t);
        } else {
            p.sessionHasExpired();
    ...
    @Path("extjs")
    @Produces(MediaType.APPLICATION_JSON)
    public CentrePack extjsFindAll(
            @QueryParam("s") String sessionId,
            @QueryParam("u") Integer userId) {
        CentrePack p = new CentrePack();
        if (isValidSession(sessionId, userId)) {
